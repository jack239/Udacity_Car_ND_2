...
/*
Using v_f = sqrt(v_i ^ 2 + 2ad), compute the final speed for acceleration given
acceleration across acceleration given distance, with initial speed v_i.
...
*/
double calc_final_speed(const double v_i, const double acceleration, const double distance) {
    double v_f{0.0};
    ...
    // infinity.
    double disc = v_i * v_i + 2 * acceleration * distance;  // <-- FIX THIS
    if (disc <= 0.0) {
        v_f = 0;  // <-- FIX THIS
    ...
    //   std::cout << "v_i, acceleration, distance: " << v_i << ", " << acceleration << ", " << distance
    //             << ",  v_f: " << v_f << std::endl;
    return v_f;
}
...
double calc_distance(const double v_i, const double v_f, const double a) {
    ...
    // YOUR CODE HERE
    if (v_i == std::numeric_limits<double>::infinity() ||
        std::isnan(v_i) ||
