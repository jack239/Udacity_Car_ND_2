    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for detection in detections:
                ...
                _, det_x, det_y, det_z, det_h, det_w, det_l, det_yaw = detection
...
def compute_performance_stats(det_performance_all):
    ...
    all_positives, true_positives, false_negatives, false_positives = np.sum(np.array(pos_negs), axis=0).astype(float)
    print(f"all_positives {all_positives}, true_positives {true_positives}, false_negatives {false_negatives}, false_positives {false_positives}")
