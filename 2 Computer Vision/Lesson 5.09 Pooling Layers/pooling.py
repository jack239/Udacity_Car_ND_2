...
def get_paddings(array, pool_size, pool_stride):
    ...
    """
    get_pad = lambda x : (x // pool_stride) * pool_stride + pool_size - x
    wpad = get_pad(array.shape[1])
    hpad = get_pad(array.shape[2])
...
def get_output_size(shape, pool_size, pool_stride):
    ...
    """
    get_new_size = lambda x: (x - pool_size) // pool_stride + 1
    new_w = get_new_size(shape[1])
    new_h = get_new_size(shape[2])
