...
double Probability(Eigen::MatrixXd X, Eigen::MatrixXd Q, Eigen::MatrixXd S){
    Eigen::MatrixXd mult = -(X - Q).transpose() * S.inverse() * (X - Q);
    assert(mult.size() == 1);
    return std::exp(-mult(0, 0) / 2);
	// calculate the probibility of the point given mean and standard deviation
}
...
Eigen::MatrixXd convertToEigen(const PointT& point){
...
using EigenPoints = std::vector<Eigen::MatrixXd>;
Eigen::MatrixXd getMean(const EigenPoints& points) {
    Eigen::MatrixXd mean = Eigen::MatrixXd::Zero(2, 1);
...
Eigen::MatrixXd getDeviation(const EigenPoints& points, const Eigen::MatrixXd& mean) {
    Eigen::MatrixXd deviation = Eigen::MatrixXd::Zero(2, 2);
    for (const auto& point : points) {
        Eigen::MatrixXd xq = point - mean;
        deviation += xq * xq.transpose();
    }
    for (size_t x: {0, 1}) {
        for (size_t y: {0, 1}) {
            deviation(x, y) = mean(x, y) / points.size();
        }
    }
...
Cell PDF(PointCloudT::Ptr input, int res, pcl::visualization::PCLVisualizer::Ptr& viewer){
    ...
    for (const auto& point : input->points) {
        points.emplace_back(convertToEigen(point));
    }
	auto Q = getMean(points);

	// Calculate the 2 x 2 matrix S, which is standard deviation of the input points
	Eigen::MatrixXd S = getDeviation(points, Q);
...
void NewtonsMethod(PointT point, double theta, Cell cell, Eigen::MatrixBase<Derived>& g_previous, Eigen:: MatrixBase<Derived>& H_previous){
	...
	// Get the Q and S matrices from cell, invert S matrix
    const auto& Q = cell.Q;
    const auto& S = cell.S;
    auto SInv = S.inverse();

	// make a 2 x 1 matrix from input point
	auto X = convertToEigen(point);

	// calculate matrix q from X and Q
    Eigen::MatrixXd q = X - Q;
	
	// calculate the 3 2 x 1 partial derivative matrices
	// each with respect to x, y, and theta
    std::vector<Eigen::MatrixXd> pd(3, Eigen::MatrixXd::Zero(2,1));
    pd[0](0,0) = 1.0;
    pd[1](1,0) = 1.0;
    double cosTheta = std::cos(theta);
    double sinTheta = std::sin(theta);
    pd[2](0,0) = -point.x * sinTheta - point.y * cosTheta;
    pd[2](1,0) =  point.x * cosTheta -point.y * sinTheta;



    // calcualte the 1 x 1 exponential matrix which uses q, and S inverse
//    Eigen::MatrixXd EXP(1,1);
    double EXP = exp(-0.5 * (q.transpose() * SInv * q)(0,0) );


    // calculate the matrix g which uses q, exponential, S inverse, and partial derivatives
	Eigen::MatrixXd g(3,1);
	g << Eigen::MatrixXd::Zero(3,1);
    for (size_t x = 0; x < 3; ++x) {
        g(x, 0) = (q.transpose() * SInv * pd[x] * EXP)(0,0);
    }


    // TODO: calculate the 2 x 1 second order partial derivative matrix

	// calculate the matrix H which uses q, exponential, S inverse, partial derivatives, and second order partial derivative
	Eigen::MatrixXd H(3,3);
	H << Eigen::MatrixXd::Zero(3,3);
    std::vector<Eigen::MatrixXd> qpd;
    for (size_t x = 0; x < 3; ++x) {
        qpd.emplace_back(-q.transpose()*SInv*pd[x]);
    }
    for (size_t x = 0; x < 3; ++x) {
        for (size_t y = 0; y < 3; ++y) {
            H(x,y) = (-EXP*(qpd[x]*qpd[y])+(-pd[y].transpose()*SInv*pd[x]))(0,0);
        }
    }
	H_previous += H;
	g_previous += g;
